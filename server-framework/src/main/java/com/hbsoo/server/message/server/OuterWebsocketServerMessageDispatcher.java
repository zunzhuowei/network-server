...
public class OuterWebsocketServerMessageDispatcher extends WebsocketServerMessageDispatcher {
    ...
    public void onMessage(ChannelHandlerContext ctx, HBSPackage.Decoder decoder) {
        ...
        handlers.values().stream()
                .filter(handler -> {
                    if (!(handler instanceof OuterWebsocketServerMessageDispatcher)) {
                        return false;
                    }
                ...
                .findFirst().ifPresent(handler -> ((OuterWebsocketServerMessageDispatcher) handler).onMessage(ctx, decoder));
    }
