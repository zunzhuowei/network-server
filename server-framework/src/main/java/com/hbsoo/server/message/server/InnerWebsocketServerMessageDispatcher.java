...
public class InnerWebsocketServerMessageDispatcher extends WebsocketServerMessageDispatcher {
    ...
    public void onMessage(ChannelHandlerContext ctx, HBSPackage.Decoder decoder) {
        ...
        handlers.values().stream()
                .filter(handler -> {
                    if (!(handler instanceof InnerWebsocketServerMessageDispatcher)) {
                        return false;
                    }
                ...
                .findFirst().ifPresent(handler -> ((InnerWebsocketServerMessageDispatcher) handler).onMessage(ctx, decoder));
    }
